...
class CodespacesAdvanced:
    ...
    def get_codespace_metrics(self, codespace_name: str = None) -> Dict:
        ...
        try:
            ...
            cmd = ['codespace', 'list', '--json', 'name,repository,state,displayName,gitStatus,machineName,lastUsedAt,createdAt,owner']
    ...
    def _get_individual_codespace_metrics(self, codespace_data: Dict) -> Dict:
        ...
        metrics = {
            ...
            'repository': codespace_data.get('repository', 'Unknown') if isinstance(codespace_data.get('repository'), str) else codespace_data.get('repository', {}).get('full_name', 'Unknown'),
            'state': codespace_data.get('state', 'Unknown'),
            'machine_type': codespace_data.get('machineName', codespace_data.get('machine', {}).get('name', 'Unknown')),
            'created_at': codespace_data.get('createdAt', codespace_data.get('created_at', 'Unknown')),
            'last_used_at': codespace_data.get('lastUsedAt', codespace_data.get('last_used_at', 'Unknown')),
        }
        ...
        # Try to get storage information (only for running codespaces)
        if metrics['state'] == 'Available':
            try:
                storage_info = self._get_codespace_storage(metrics['name'])
                metrics.update(storage_info)
            except Exception as e:
                self.logger.debug(f"Could not get storage for {metrics['name']}: {e}")
                metrics['storage_used_mb'] = 0
                metrics['storage_quota_mb'] = 'Unknown'
        else:
            # Estimate storage for shutdown codespaces (typical usage)
            metrics['storage_used_mb'] = 1024  # Estimate 1GB for shutdown codespaces
            metrics['storage_quota_mb'] = 32000  # 32GB typical quota
    ...
    def _get_codespace_storage(self, codespace_name: str) -> Dict:
        ...
        try:
            ...
            cmd = ['gh', 'codespace', 'ssh', '--codespace', codespace_name,
                   '--', 'df', '-m', '/']
